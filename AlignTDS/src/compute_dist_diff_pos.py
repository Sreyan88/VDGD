...
if __name__ == "__main__":
    ...
    i2i_path = f"..//AlignTDS/saved_logits/just_eval_1000/{pair_name}/{pair_name}-i2i.pkl"
    i2b_path = f"..//AlignTDS/saved_logits/just_eval_1000/{pair_name}_tp/{pair_name}-i2b.pkl"
    ...
    print(len(i2b_data))
    curves = {}
    for eid in tqdm(range(pair_len)):
    ...
    # print([curves[i][:3+i] for i in [0,1]])

    # with open(f"..//AlignTDS/saved_logits/curves/{pair_name}_curves.pkl", "wb") as f:
    ...
    for mt in metrics: 
        ...
        plt.plot(x[:100], y[:100], label=f'{mt}', color='blue', linestyle='-')
        plt.xlabel('Position')
        plt.ylabel('Score')
